...
const Catalog: React.FC<CatalogProps> = React.memo(({ searchQuery = "", isVintageOnly = false, user, isIncognito = false }) => {
    ...
    const [sortBy, setSortBy] = React.useState<'name' | 'id' | 'offers' | 'completion'>('name');
    ...
    return (
        <div className="space-y-2 md:space-y-3 animate-in fade-in duration-1000">
            ...
            <div className="relative overflow-hidden flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between rounded-2xl md:rounded-3xl border border-white/5 bg-black/25 p-4 md:p-6 backdrop-blur-2xl shadow-2xl">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 w-full lg:w-auto relative z-10">
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2 w-full sm:w-auto">
                        <div className="h-[38px] sm:h-[42px] p-1 rounded-xl bg-black/40 border border-white/10 grid grid-cols-4 gap-1 flex-1 sm:flex-initial sm:min-w-[240px] items-center">
                            <button
                                onClick={() => setSortBy('name')}
                                className={`h-full px-1.5 sm:px-2.5 rounded-lg text-[9px] sm:text-[10px] font-black uppercase tracking-wider transition-all duration-200 flex items-center justify-center whitespace-nowrap ${
                                    sortBy === 'name' 
                                        ? (isVintageOnly 
                            ...
                            <button
                                ...
                                title="Ordenar por Número de Ofertas"
                            >
                                OFERTAS
                            </button>
                            <button
                                ...
                                title="Ordenar por Colección del Set"
                            >
                                SET
                            </button>
                        </div>
            ...
            {viewMode === 'cronos' ? (
                <CronosView
                    selectedCronosA={selectedCronosA}
                    setSelectedCronosA={setSelectedCronosA}
                    cronosSearchA={cronosSearchA}
                    setCronosSearchA={setCronosSearchA}
                    selectedCronosB={selectedCronosB}
                    setSelectedCronosB={setSelectedCronosB}
                    cronosSearchB={cronosSearchB}
                    setCronosSearchB={setCronosSearchB}
                    products={products}
                    availableShops={availableShops}
                    activeCronosShops={activeCronosShops}
                    setActiveCronosShops={setActiveCronosShops}
                    cronosStats={cronosStats}
                    loadingCronosA={loadingCronosA}
                    loadingCronosB={loadingCronosB}
                    chartData={chartData}
