...
interface VintageCalibratorModalProps {
    isOpen: boolean;
    coords: SwordCoords;
...
/** Fase AAA-4a: extraido de Config.tsx ("Vintage Sword Light Ray Calibrator Modal"). */
export default function VintageCalibratorModal({ isOpen, coords, onChange, onSave, onReset, onClose }: VintageCalibratorModalProps) {
                    ...
                    <div className="flex flex-col items-center justify-center gap-4 bg-black/40 border border-white/5 p-6 rounded-3xl">
                        ...
                        <div className="relative w-64 h-64 border border-white/10 rounded-2xl overflow-hidden bg-[#050608] flex items-center justify-center shadow-inner">
                            <PowerSwordLoader
                                ...
                                disableRandom={true}
                                size={250}
                                vintageGuardX={coords.gX}
                                vintageGuardY={coords.gY}
                                vintageTipX={coords.tX}
                                vintageTipY={coords.tY}
                            ...
                            <svg viewBox="0 0 250 250" className="absolute inset-0 w-full h-full pointer-events-none">
                                ...
                                <circle cx={coords.gX} cy={coords.gY} r="4" fill="#10B981" stroke="white" strokeWidth="1" />
                                <text x={coords.gX + 6} y={coords.gY + 3} fill="#10B981" fontSize="8" fontWeight="bold">Empuñadura ({coords.gX.toFixed(1)}, {coords.gY.toFixed(1)})</text>

                                {/* Tip indicator */}
                                <circle cx={coords.tX} cy={coords.tY} r="4" fill="#EF4444" stroke="white" strokeWidth="1" />
                                <text x={coords.tX + 6} y={coords.tY + 3} fill="#EF4444" fontSize="8" fontWeight="bold">Punta ({coords.tX.toFixed(1)}, {coords.tY.toFixed(1)})</text>

                                {/* Axis line */}
                                <line x1={coords.gX} y1={coords.gY} x2={coords.tX} y2={coords.tY} stroke="rgba(245,158,11,0.3)" strokeDasharray="3" strokeWidth="1.5" />
                            </svg>
                        </div>
