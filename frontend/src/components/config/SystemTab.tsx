...
import {
    Database, Lock, AlertCircle, Target, Globe, Repeat, ChevronDown,
    CheckCircle2, RefreshCw, Download, Package, Sparkles, Settings, ShieldAlert,
    FileSpreadsheet, Trash2, Zap, Copy, Check, X, ShieldCheck, Bell, ExternalLink
} from 'lucide-react';
...
export default function SystemTab({
    ...
    const [lastExcelSync, setLastExcelSync] = useState<string | null>(() => localStorage.getItem('motu_last_excel_sync'));
    const [lastFinopsMaintenance, setLastFinopsMaintenance] = useState<string | null>(() => localStorage.getItem('motu_last_finops_maintenance'));
    const [cacheStorageSize, setCacheStorageSize] = useState<string | null>(null);
            ...
            if (typeof navigator !== 'undefined' && 'storage' in navigator && 'estimate' in navigator.storage) {
                try {
                    ...
                    if (usage !== undefined) {
                        ...
                        setCacheStorageSize(`${mb} MB`);
                    }
                } catch (e) {
                    console.warn('Could not estimate cache storage:', e);
                }
            }
        };
        calculateStorage();
    ...
    const formatLastRun = (isoStr: string | null) => {
        if (!isoStr) return 'Sin registros recientes';
        try {
    ...
    const handleSyncExcelWithTelemetry = () => {
        handleSyncExcel();
        const now = new Date().toISOString();
        localStorage.setItem('motu_last_excel_sync', now);
        setLastExcelSync(now);
    };

    const handleRunMaintenanceWithTelemetry = () => {
        handleRunMaintenance();
        const now = new Date().toISOString();
        localStorage.setItem('motu_last_finops_maintenance', now);
        setLastFinopsMaintenance(now);
    };
    ...
    return (
        ...
        >
            ...
            {/* SECCIÓN 1: PREFERENCIAS DE COLECCIÓN & ENTORNO */}
            <div className="space-y-4">
                <div className="flex items-center gap-2 px-1">
                    <span className="h-1.5 w-1.5 rounded-full bg-cyan-400"></span>
                    <h3 className="text-xs font-black uppercase tracking-[0.2em] text-white/60">
                        Preferencias de Colección & Entorno
                    </h3>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {/* Telegram Push Smart Filter */}
                    <div className="glass border border-cyan-500/30 p-6 rounded-3xl space-y-4 bg-cyan-500/[0.03] relative flex flex-col justify-between">
                        <div className="space-y-4">
                        ...
                        <div className="text-[9px] text-cyan-400/80 font-bold uppercase tracking-wider flex items-center gap-1.5 pt-2 border-t border-white/5">
                    ...
                    <div className="glass border border-brand-primary/30 p-6 rounded-3xl space-y-4 bg-brand-primary/5 flex flex-col justify-between">
                        <div className="space-y-4">
                            ...
                            <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl">
                                ...
                                <button
                                    ...
                                    className={`relative h-5 w-10 rounded-full transition-all cursor-pointer ${userSettings?.is_public_showcase ? 'bg-brand-primary' : 'bg-white/10'}`}
                                >
                            ...
                            {userSettings?.is_public_showcase && (
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    <button
                                        ...
                                        className="w-full bg-brand-primary/10 hover:bg-brand-primary text-brand-primary hover:text-white border border-brand-primary/25 px-3 py-2.5 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 cursor-pointer"
                                    >
                                        <Repeat className="h-3 w-3 animate-pulse text-brand-primary" />
                                        Copiar Enlace
                                    </button>
                                    <a
                                        href={`/santuario/${userSettings.username}`}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="w-full bg-white/5 hover:bg-white/10 text-white/80 hover:text-white border border-white/10 px-3 py-2.5 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 cursor-pointer"
                                        title="Abrir en pestaña nueva para verificar la vista externa"
                                    >
                                        <ExternalLink className="h-3 w-3 text-brand-primary" />
                                        Ver Santuario
                                    </a>
                                </div>
                            )}
                    ...
                    <div className="glass border border-brand-primary/30 p-6 rounded-3xl space-y-4 bg-brand-primary/5 flex flex-col justify-between">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {[
                                    ...
                                    <button
                                        ...
                                        className={`flex items-center justify-between px-4 py-3 rounded-2xl border transition-all cursor-pointer ${userSettings?.location === country.code
                        ...
                        {savingSettings && (
                            <div className="flex items-center gap-2 text-[8px] font-black text-brand-primary uppercase animate-pulse justify-center pt-2">
            ...
            {/* SECCIÓN 2: RENDIMIENTO & ALMACENAMIENTO */}
            <div className="space-y-4">
                <div className="flex items-center gap-2 px-1">
                    <span className="h-1.5 w-1.5 rounded-full bg-brand-primary"></span>
                    <h3 className="text-xs font-black uppercase tracking-[0.2em] text-white/60">
                        Rendimiento, Caché & Servidor
                    </h3>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                ...
                                {downloadStatus.active ? (
                                    <div className="space-y-2">
                                        ...
                                        <button
                                            ...
                                            className="w-full bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white border border-red-500/20 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all cursor-pointer"
                                        >
                                            Cancelar Descarga
                                        </button>
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="text-[9px] font-bold text-white/40 uppercase text-center mt-1">
                                            ...
                                            {cacheStorageSize && (
                                                <span className="text-white/60 ml-1">({cacheStorageSize})</span>
                                            )}
                                        </div>
                    ...
                    <div className="glass border border-white/10 p-6 rounded-3xl space-y-4 bg-white/5 flex flex-col justify-between" data-render={forceRender}>
                        <div className="space-y-4">
                    ...
                    {/* Parámetros Activos del Servidor (.env) */}
                    <div className="glass border border-white/10 p-6 rounded-3xl space-y-4 bg-white/[0.02] relative flex flex-col justify-between">
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3 text-white/80 font-bold uppercase tracking-widest text-xs">
                                    <Lock className="h-4 w-4 text-amber-400" />
                                    Parámetros del Servidor (.env)
                                </div>
                            ...
                            <p className="text-[10px] text-white/60 font-medium leading-relaxed">
                                Variables globales de scraping, umbrales financieros y centinela fijadas en la infraestructura del servidor.
                            </p>

                            <div className="grid grid-cols-2 gap-2 pt-1 text-[10px]">
                                <div className="p-3 bg-black/40 border border-white/5 rounded-2xl flex flex-col justify-between">
                                    <span className="text-white/40 uppercase font-black text-[8px]">ROI Grial Mínimo</span>
                                    <span className="font-mono font-bold text-yellow-400 text-xs mt-1">≥ 50%</span>
                                </div>
                                <div className="p-3 bg-black/40 border border-white/5 rounded-2xl flex flex-col justify-between">
                                    <span className="text-white/40 uppercase font-black text-[8px]">Umbral Grial</span>
                                    <span className="font-mono font-bold text-yellow-400 text-xs mt-1">≥ 150 €</span>
                                </div>
                                <div className="p-3 bg-black/40 border border-white/5 rounded-2xl flex flex-col justify-between">
                                    <span className="text-white/40 uppercase font-black text-[8px]">Delay Scrapers</span>
                                    <span className="font-mono font-bold text-blue-400 text-xs mt-1">10 seg (Auto)</span>
                                </div>
                                <div className="p-3 bg-black/40 border border-white/5 rounded-2xl flex flex-col justify-between">
                                    <span className="text-white/40 uppercase font-black text-[8px]">Modo Sigilo</span>
                                    <span className="font-mono font-bold text-emerald-400 text-xs mt-1 flex items-center gap-1">
                                        <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse"></span>
                                        Playwright Stealth
                                    </span>
                                </div>
                                <div className="col-span-2 p-3 bg-black/40 border border-white/5 rounded-2xl flex items-center justify-between">
                                    <div className="space-y-0.5">
                                        <span className="text-white/40 uppercase font-black text-[8px] block">Vigilancia Sentinel</span>
                                        <span className="text-[10px] text-white/70 font-medium">Umbral caída de precio</span>
                                    </div>
                                    <span className="font-mono font-bold text-orange-400 text-xs">15%</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {/* SECCIÓN 3: INFRAESTRUCTURA & BÓVEDA (SHIELD LAYER 2) */}
            {isAdmin && (
                <div className="space-y-4">
                    <div className="flex items-center justify-between px-1">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="text-xs font-black uppercase tracking-[0.2em] text-white/60">
                                Infraestructura & Bóveda Digital (Shield Layer 2)
                            </h3>
                        </div>
                        <span className="text-[8px] font-mono px-2 py-0.5 rounded-full bg-blue-900/30 text-blue-400 border border-blue-800/50">
                            ADMIN EXCLUSIVE
                        </span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div className={`glass border p-6 rounded-3xl group transition-all flex flex-col justify-between ${
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-2 p-3 rounded-2xl bg-white/[0.02] border border-white/5 text-[10px]">
                        ...
                        <div className="glass border border-white/10 p-6 rounded-3xl group hover:bg-white/5 transition-all flex flex-col justify-between">
                            <div className="space-y-4">
                                <div className="flex items-start justify-between">
                                    <div className="flex items-center gap-4">
                                ...
                                <div className="p-3 rounded-2xl bg-white/[0.02] border border-white/5 text-[9px] text-white/60 space-y-1">
                                    <div className="text-white/40 uppercase font-black text-[8px]">Última purificación:</div>
                                    <div className="font-mono text-brand-primary font-bold">{formatLastRun(lastFinopsMaintenance)}</div>
                                </div>
                            </div>
                            <div className="pt-4">
                                <button
                                    onClick={handleRunMaintenanceWithTelemetry}
                                    disabled={runningMaintenance}
                                    className={`w-full bg-brand-primary/10 hover:bg-brand-primary text-brand-primary hover:text-white border border-brand-primary/20 px-4 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-lg shadow-brand-primary/0 hover:shadow-brand-primary/20 cursor-pointer ${runningMaintenance ? 'opacity-50 cursor-not-allowed' : ''}`}
                                >
                        ...
                        <div className="glass border border-white/10 p-6 rounded-3xl group hover:bg-white/5 transition-all flex flex-col justify-between">
                            <div className="space-y-4">
                                <div className="flex items-start justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] text-white/60 font-mono">Sincronización {user?.username || 'David'} MOTU</p>
                                        </div>
                                    </div>
                                    <div className="flex flex-col items-end">
                                ...
                                <div className="p-3 rounded-2xl bg-white/[0.02] border border-white/5 text-[9px] text-white/60 space-y-1">
                                    <div className="text-white/40 uppercase font-black text-[8px]">Última sincronización:</div>
                                    <div className="font-mono text-green-400 font-bold">{formatLastRun(lastExcelSync)}</div>
                                </div>
                            </div>
                            <div className="pt-4">
                                <button
                                    onClick={handleSyncExcelWithTelemetry}
                                    disabled={syncingExcel}
                                    className={`w-full bg-green-500/10 hover:bg-green-500 text-green-400 hover:text-white border border-green-500/20 px-4 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-lg shadow-green-500/0 hover:shadow-green-500/20 cursor-pointer ${syncingExcel ? 'opacity-50 cursor-not-allowed' : ''}`}
                                >
                        ...
                        {/* --- CALIBRACIÓN DE ANIMACIONES Y PANTALLA DE CARGA --- */}
                        <div className="glass border border-purple-500/20 p-6 rounded-3xl space-y-4 bg-purple-500/[0.02] flex flex-col justify-between">
                            <div className="space-y-4">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3 text-purple-400 font-bold uppercase tracking-widest text-xs">
                                        <Zap className="h-4 w-4" />
                                        Calibrador de Animaciones
                                    </div>
                                    <span className="text-[9px] font-mono px-2 py-0.5 rounded-full bg-purple-500/10 border border-purple-500/20 text-purple-300">
                                        PANTALLA DE CARGA
                                    </span>
                                </div>
                                <p className="text-[10px] text-white/65 font-bold uppercase leading-tight">
                                    Calibra la posición exacta de los haces de luz sobre el báculo de Skeletor o la espada de He-Man en el loader de inicio.
                                </p>
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-2">
                                <button
                                    onClick={() => setShowModernCalibrator(true)}
                                    className="bg-purple-500/10 hover:bg-purple-500 text-purple-400 hover:text-black border border-purple-500/25 px-3 py-2.5 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-purple-500/0 hover:shadow-purple-500/20"
                                >
                                ...
                                <button
                                    ...
                                    className="bg-amber-500/10 hover:bg-amber-500 text-amber-400 hover:text-black border border-amber-500/25 px-3 py-2.5 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-amber-500/0 hover:shadow-amber-500/20"
                                >
                                    <Settings className="h-3.5 w-3.5" />
                                    Espada Vintage
                                </button>
                        ...
                        {/* --- PURIFICATION (ADMIN POWER / DANGER ZONE) --- */}
                        <div className="glass border border-red-500/30 p-6 rounded-3xl space-y-4 bg-red-500/5 flex flex-col justify-between">
                            <div className="space-y-4">
                            ...
                            <button
                                ...
                                className="w-full bg-red-500/10 hover:bg-red-500/20 text-red-500 border border-red-500/30 py-4 rounded-2xl font-black uppercase tracking-widest text-xs transition-all flex items-center justify-center gap-2 cursor-pointer"
                            >
