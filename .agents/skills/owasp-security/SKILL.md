...
### Prevention Patterns (FastAPI / SQLAlchemy)
```python
...
# ✅ GOOD: Verify ownership & role-based checks (RBAC)
from fastapi import Depends, HTTPException, status

async def get_current_user(token: str = Depends(oauth2_scheme)):
    # Decoding JWT & fetching user
...
```python
# ❌ BAD: Can enumerate other users' items by guessing serial IDs
# GET /api/auctions/1001

# ✅ GOOD: Verify item ownership and access permission
...
async def get_auction(
    ...
    auction = db.query(Auction).filter(
        ...
        Auction.user_id == current_user.id  # Enforce ownership in filter
    ).first()
    
    if not auction:
        raise HTTPException(status_code=404, detail="Auction not found or unauthorized")
    return auction
```
