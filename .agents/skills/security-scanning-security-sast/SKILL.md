...
# For flat config, run:
npx eslint . --format json > eslint-security.json
```
...
### Python: FastAPI & SQLAlchemy Specific Rules

- **CORS Wildcards**: Avoid `allow_origins=["*"]` when `allow_credentials=True` is set in `CORSMiddleware`.
- **SQL Injection**: Avoid passing formatted string templates into SQLAlchemy raw queries. Use `text()` and bind parameters.
- **Pydantic Model Parsing**: Avoid bypassing schemas during raw deserialization; ensure proper Pydantic validator setups.
