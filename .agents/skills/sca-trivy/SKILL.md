...
Scan a container image or filesystem for vulnerabilities:
...
# or via scoop:
scoop install trivy
# or run via Docker on Windows:
docker pull aquasec/trivy:latest

# Scan local project filesystem (Vulnerabilities and Secrets)
trivy fs .

# Scan specific packages inside our folder structure
trivy fs --scanners vuln ./frontend/package-lock.json
trivy fs --scanners vuln ./requirements.txt

# Scan IaC files / Dockerfiles for misconfigurations
trivy config .

# Run via Docker on Windows (using current directory mapping)
