...
- [x] **Phase 76: Compactación Estadística FinOps e Incursión Universal CDP (07/07/2026)**
  ...
  - [x] **Purgas Inteligentes**: Configurado el purgado de ofertas detalladas inactivas para productos muertos e historial antiguo para productos vivos (solventando un error de `ForeignKeyViolation` en Postgres limpiando el historial hijo `PriceHistoryModel` en primera instancia).
  - [x] **Mantenimiento en Daily Scan (Tolerancia a Fallos & Telegram)**: Enganchado el mantenimiento al inicio de `daily_scan.py` con try-except defensivo para que, en caso de fallar o tardar, no interrumpa el proceso de scraping nocturno de GitHub Actions. Al finalizar el Daily Scan completo, se envía un reporte push resumido a Telegram con las ofertas procesadas, nuevas inyectadas, el total de ítems enviados al Purgatorio y los errores.
...
- [x] **Fase AAA: Auditoría y Elevación a Triple-A (21/07/2026)** — rama `refactor/aaa-uplift`. Reporte completo en [REPORTE_MEJORAS_AAA.md](REPORTE_MEJORAS_AAA.md).
  - [x] **⚠️ Corrección sobre la Fase 25**: la "Estandarización de `ORACULO_API_KEY` en todos los clientes frontend" (línea 259 de este documento) resultó ser una vulnerabilidad crítica — esa clave quedaba compilada en el bundle JS público y, combinada con un bypass de login (`password == ORACULO_API_KEY`), permitía a cualquier persona con el navegador abierto obtener acceso de administrador. **Revertido**: el frontend ya no envía `ORACULO_API_KEY` en ningún caso; la clave pasa a ser exclusivamente servidor-a-servidor (scrapers/scripts). El acceso de administrador es ahora siempre JWT obtenido por login real (email + contraseña).
  - [x] **Seguridad**: IDOR cerrado en `collection`, `vault`, `logistics`, `users` (el hallazgo más grave: cualquiera podía activar el escaparate público de otro usuario sin autenticarse); path traversal corregido en `/api/vault/stage`; CORS restringido; rate limiting en auth; guardián que aborta el arranque en producción si los secretos siguen en su valor de ejemplo.
  - [x] **Arquitectura**: router real (`react-router-dom`) sustituyendo `activeTab` + *keep-alive* — cada página monta/desmonta de verdad al navegar en vez de acumularse oculta en el DOM.
  - [x] **Rendimiento**: `manualChunks` en Vite para vendors cacheables, precarga de páginas al pasar el ratón por el menú, eliminadas 16 imágenes duplicadas/sin usar de `frontend/src/assets` (18 MB → 412 KB), y cierre del hueco por el que las imágenes nuevas descubiertas por los scrapers de tienda nunca se convertían a WebP (solo cubría el importador antiguo de Excel).
  - [x] **Mantenibilidad**: troceados `Config.tsx` (3101→886 líneas, -71%), `Purgatory.tsx` (2276→923, -59%) y `Catalog.tsx` (1965→887, -55%) en 24 componentes nuevos bajo `frontend/src/components/{config,purgatory,catalog}/` — extracción mecánica 1:1, sin re-arquitecturar lógica de negocio. Ver [REPORTE_MEJORAS_AAA.md](REPORTE_MEJORAS_AAA.md) Ola 4a y [architecture_map.md](technical/architecture_map.md) §4 para el detalle completo.
...
- [x] **Phase 88: Living Lore Engine MOTU a Coste Cero, Inmutabilidad de Base de Datos y Grimorio de Configuración (20/08/2026)**
  - [x] **Motor Cosechador Determinista a Coste 0 (`LoreHarvesterService`)**: Integración con MediaWiki API de Wiki Grayskull (Fandom) y extracción por expresiones regulares sin consumo de tokens ni créditos IA.
  - [x] **Regla de Inmutabilidad y Caché Permanente**: Una vez que un personaje tiene lore en base de datos (`character_lore`), jamás se vuelve a cosechar automáticamente de internet. Las 507 figuras heredan lore canónico instantáneamente a 0ms.
  - [x] **Reglas Canónicas Obligatorias**: Frase de He-Man fijada estrictamente en *"¡Por el poder de Grayskull, yo tengo el poder!"*, Anti-Eternia He-Man clasificado como villano (*Guerreros del Mal / Snake Mountain*), y perfiles diferenciados para variantes clave (*He-Skeletor, He-Ro, Great Black Wizard*).
...
- [x] **Phase 100: Auditoría Gentle AI — Rendimiento Global, Privacidad Incógnito e Integridad del Ecosistema (22/09/2026)**
  - [x] **Bloque 1 (Privacidad y Modo Incógnito)**: Propagación universal de `isIncognito` a Dashboard, widgets y modales. Difuminado total en métricas de rentabilidad, cotizaciones de mercado y ofertas. Bloqueo de inspección en casilla de coste manual con opacidad y desactivación de tooltip nativo (`title="•••"`).
  - [x] **Bloque 2 (Rendimiento Backend y Base de Datos)**: Desbloqueo del bucle asyncio en FastAPI delegando endpoints a threadpool (`def`). Erradicación del antipatrón N+1 en `/api/collection` mediante precarga de ofertas en una sola query agrupada. Paginación SQL con `LIMIT`/`OFFSET`. Concurrencia SQLite optimizada con pragmas WAL, `synchronous=NORMAL`, `busy_timeout=30000` y micro-commits en scrapers. Índices de rendimiento formalizados en migración Alembic `8f1e2d3c4b5a`.
  - [x] **Bloque 3 (Rendimiento Frontend e Interactividad)**: `FoilTiltCard` acelerado a 60 FPS sin re-renders de React mediante manipulación directa de variables CSS en el DOM. Aislamiento y lazy loading de `TradingCardModal` (reducción de 94 kB en carga inicial). Prevención de fugas de RAM con política LRU de 150 elementos y revocación explícita de `URL.revokeObjectURL` en `MOTUImage`.
  - [x] **Bloque 4 (Integridad de Modelos, Tipos y Scrapers)**: Retirada de DDL manual de arranque en `universal_migrator.py` consolidando `spider_name` bajo Alembic. Alineación de `CollectionItemBase`/`CollectionItemSchema` con las columnas reales de `CollectionItemModel`. Sincronización de interfaces TypeScript (`FinancialHealth`, `DashboardStats`) con los endpoints backend. Eliminación de importación pesada no utilizada de `WallapopScraper`.
