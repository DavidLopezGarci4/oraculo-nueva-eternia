...
- ✅ **Fase 3.1 — Router real (frontend):** hecha y verificada en navegador real (no solo build). `react-router-dom` sustituye `activeTab` + *keep-alive*; cada página monta/desmonta de verdad al navegar (confirmado por JS que el DOM de una página anterior desaparece por completo). `Sidebar.tsx`/`Navbar.tsx` no se tocaron (adaptador de compatibilidad). Guard de `/purgatory` ahora declarativo en la ruta.
  - ✅ **Fase 3.2 — hecha (2026-07-21):** troceados `Config.tsx` (3101→886 líneas, -71%), `Purgatory.tsx` (2276→923, -59%) y `Catalog.tsx` (1965→887, -55%) en 24 componentes nuevos. Detalle completo en Ola 4a más abajo.
- 🟡 **Fase 4 — Rendimiento: parcial, lo de mayor impacto/menor riesgo ya hecho.**
  ...
  - 4.5 ⏸️ pendiente — virtualización de listas largas en Catálogo/Colección/Purgatorio. Evaluada y **pospuesta deliberadamente** (2026-07-21): a diferencia del troceo de 4a, es un cambio de arquitectura de renderizado que necesita datos reales para verificar visualmente (la BD de desarrollo está vacía). Recomendación técnica al retomarla: ver Ola 4/4b más abajo.
...
Esto es lo de **mayor retorno de todo el backlog**: la rama `refactor/aaa-uplift` va **54 commits por delante de `main`** (verificado con `git rev-list --count main..refactor/aaa-uplift` el 2026-07-21 — si tu copia de `Apuntes a llevar a cabo.txt` dice "~19 commits", está desactualizada; usa el `git log` real de la rama como fuente de verdad, no un número recordado de una sesión anterior) — seguridad, troceo de los 3 monolitos, y ahora también 20 CVEs de dependencias corregidos. Nada de esto protege ni corre en producción hasta que despliegues.

**Pasos, en tu máquina con Docker (auto-contenido, no dependas solo de tu archivo de notas por si quedó desactualizado):**

1. **Backup antes de tocar nada.** Local: `.\backup_db.ps1`. Producción (Supabase/Postgres): backup desde el panel de Supabase (Database → Backups) o `pg_dump` si tienes la cadena de conexión directa.
2. **Trae la rama a tu máquina**: `git fetch origin && git checkout refactor/aaa-uplift && git pull`.
3. **Rota los secretos de Ola 0** (0.A/0.B) si no lo has hecho ya — hazlo ANTES de desplegar el código nuevo, no después; son independientes de Docker.
4. **Migración Alembic**: sigue [ALEMBIC_ADOPTION.md](ALEMBIC_ADOPTION.md) paso a paso — ahora son **2 migraciones** encadenadas (`d4283e0fbed1` columnas de showcase + `feac3ac24f77` índices de rendimiento de la Ola 3e), ambas idempotentes, `alembic upgrade head` las aplica juntas.
5. **Revisa `.env` de producción** contra `.env.example` actualizado — confirma que `JWT_SECRET`/`ORACULO_API_KEY` NO son los valores de ejemplo (la app aborta el arranque si lo son, Fase 1.5) y que `EXTENSION_API_KEY` está presente (Ola 3d, guardián dual de `/api/wallapop/import`).
6. **`docker compose -f docker-compose.prod.yml up -d --build`** — esto reconstruye la imagen con `requirements.txt` actualizado (fastapi/starlette/pyjwt/pydantic-settings con los CVEs corregidos de Ola 5, ver tabla en [BASELINE_METRICS.md](BASELINE_METRICS.md)) sin que tengas que hacer nada manual con las dependencias — el `Dockerfile` las instala solas desde el fichero.
7. **Verifica**: logs del contenedor backend (deberías ver `✅ Migraciones aplicadas correctamente.`), login real con tu email+contraseña, colección, configuración. Repite la consulta SQL del paso 7 de [ALEMBIC_ADOPTION.md](ALEMBIC_ADOPTION.md) si quieres confirmar la versión de esquema aplicada.
8. **Si el repo de GitHub es público** y no lo has resuelto en Ola 0.C, hazlo ahora también.
...
### 🟡 OLA 4 — Refactor grande (sesión dedicada, alto riesgo de regresión) — 4a completa, 4b pospuesto
...
- **4b — Virtualizar listas largas** (`@tanstack/react-virtual`) en Catálogo/Colección/Purgatorio para no renderizar cientos de tarjetas de golpe. Toca `Catalog.tsx`, encaja con 4a. (4.5 / B.5) — 🟡 **evaluado y pospuesto deliberadamente (2026-07-21).** A diferencia de las extracciones mecánicas de 4a (mismo comportamiento, solo movido de fichero), virtualizar el grid responsive de `Catalog.tsx` (`grid-cols-2 sm:... lg:grid-cols-3 xl:grid-cols-4 landscape:grid-cols-3`) es un cambio de arquitectura de renderizado que necesita datos reales para verificar que no rompe el layout — la base de datos de desarrollo está vacía, así que no se pudo comprobar visualmente. **Recomendación técnica para cuando se retome:** virtualización por filas (`useVirtualizer` de `@tanstack/react-virtual` con `count = Math.ceil(items.length / columnasActuales)`), detectando el número de columnas actual vía `ResizeObserver`/`matchMedia` en vez de asumirlo fijo, y probándolo con datos reales (no la BD vacía de dev) antes de mergear. No tocar sin poder ver el resultado en un navegador con productos reales cargados.
- **4c — Adopción incompleta de la instancia `apiClient` (NO es un hueco de seguridad)** (Fase 3.4, hallazgo de la revisión documental del 2026-07-21, corregido tras verificar dos veces): `frontend/src/api/client.ts` (Fase AAA-1) registra sus interceptores de auth sobre el objeto **`axios` global** — `main.tsx` lo importa una vez al arrancar la app (`import './api/client'`), así que **todo** módulo que use `axios` directamente ya recibe `Authorization: Bearer <jwt>` y limpieza de sesión en 401, lo importe o no explícitamente. Confirmado leyendo `client.ts` línea a línea (el propio comentario del código lo dice: "lo usan collection.ts, products.ts, dashboard.ts, cart.ts, LoginPage…"). Lo que sí queda inconsistente: solo `dashboard.ts`/`wallapop.ts` usan la instancia nombrada `apiClient` (con `baseURL: '/api'` preconfigurada); el resto redeclara `const API_BASE_URL = '/api'` por su cuenta. 🟢 bajo esfuerzo, solo limpieza/DRY — migrar cada `api/*.ts` a importar `apiClient` y quitar el `API_BASE_URL` duplicado; verificar con `tsc` + build + smoke test en navegador.
- **4d — CI/pytest no recogen 4 archivos de test existentes, y uno de ellos está roto** (hallazgo de la revisión documental del 2026-07-21): `pyproject.toml` limita `python_files = ["test_api_*.py"]`, así que `tests/integration/test_phase0_migration.py`, `tests/unit/test_matcher_precision.py`, `tests/unit/test_specific_purgatory_cases.py` y `tests/unit/test_wallapop_signed_api.py` **nunca se ejecutan** ni en local (`pytest tests/`) ni en CI (`ci.yml` corre exactamente `pytest tests/ -v`) — sus nombres no encajan en el patrón, pytest no los ve. Ejecutados explícitamente por ruta para comprobar su estado real: **3 en verde, pero `test_phase0_migration.py::test_migration_logic` FALLA** (`AttributeError: 'NoneType' object has no attribute 'acquired'` — un `CollectionItemModel` que el test espera encontrar no existe, probablemente por deriva del esquema/datos de prueba desde que se escribió). Detalle completo en [architecture_map.md](technical/architecture_map.md) §8. 🟡 esfuerzo medio — antes de ampliar el filtro de pytest (`python_files`) para que CI los recoja, hay que arreglar `test_migration_logic` o confirmar que el test (no el código de migración real) es el que quedó desactualizado.
...
- **B.9** — ✅ **COMPLETA (2026-07-21).** Revisados `docs/DOCUMENTACION_MAESTRA.md`, `docs/MASTER_ROADMAP.md`, `docs/README.md`, `docs/CONTEXT.md`, `docs/BASELINE_METRICS.md` y `docs/technical/architecture_map.md` por afirmaciones desfasadas tras el troceo de Ola 4a y el bump de dependencias de Ola 5. Encontrada y corregida una afirmación obsoleta en `MASTER_ROADMAP.md` (línea "Pendiente: trocear Config.tsx/Purgatory.tsx/Catalog.tsx..."). `DOCUMENTACION_MAESTRA.md` y `CONTEXT.md` no tenían afirmaciones de estructura de archivos que requirieran cambio (son doc de arquitectura conceptual/dominio de negocio, no de estructura de ficheros). `architecture_map.md` §4 y §8 actualizados con la nueva estructura de `frontend/src/components/` y el conteo de tests real.
...
**Diagnóstico global (ORIGINAL, congelado — así estaba la app antes de empezar este backlog).** No lo edites para reflejar avances: para saber dónde está la app **hoy de verdad**, ve a **[Estado de ejecución](#estado-de-ejecución-actualizado-2026-07-21)** al principio de este documento. Se deja aquí tal cual porque explica el **porqué** de la priorización de fases de más abajo:

| Eje | Estado en el diagnóstico original | Bloqueante para AAA | Estado real hoy (ver "Estado de ejecución") |
|-----|---------------|---------------------|------|
| 🔴 **Seguridad** | Modelo de autenticación efectivamente inexistente en el borde real. Clave maestra hardcodeada en el bundle del navegador. IDOR generalizado. CORS abierto. Sin cabeceras de seguridad. | **SÍ — crítico** | ✅ Fase 1 completa (código). ⏸️ Sin desplegar/rotar secretos todavía — ver Ola 0/1. |
| 🟠 **Arquitectura** | Páginas monolíticas (`Config.tsx` 3072 líneas, `Purgatory.tsx` 2253, `Catalog.tsx` 1965). Sin router real; navegación por `activeTab` + *keep-alive* que mantiene todas las secciones montadas. Auth 100% client-side. | Parcial | ✅ Router real (Fase 3.1) y los 3 monolitos troceados (Ola 4a). Auth sigue siendo JWT (correcto, no client-side puro). |
| 🟡 **Rendimiento** | Bundle principal único de **1.23 MB**, `dist` de **18 MB**, **17 MB de PNG** sin optimizar (con duplicados). Todas las secciones visitadas quedan vivas en memoria y siguen refetcheando. | Sí (métricas AAA) | ✅ Bundle entry 140.86→88.70 KB gzip, `dist` 18→11 MB, imágenes 17.1 MB→412 KB. ⏸️ Virtualización de listas (4b) pospuesta. Lighthouse real pendiente de despliegue. |
