...
async def verify_tradeinn_two_phase():
    async with async_playwright() as p:
        print("🚀 Lanzando navegador para verificación de dos fases...")
        ...
        print(f"🧭 Intentando Fase A (Directa): {url}...")
        try:
            await page.goto(url, wait_until="domcontentloaded", timeout=30000)
            await asyncio.sleep(3)
            
            if await page.locator('li.js-content-buscador_li').count() > 0:
                print("✅ ÉXITO en Fase A.")
            else:
                print("🔄 Fase A fallida. Iniciando Fase B (Interactiva)...")
                ...
                print("🚀 Búsqueda interactiva enviada.")
                await asyncio.sleep(5)
        except Exception as e:
            print(f"⚠️ Error en estrategia: {str(e)[:50]}")
        ...
        try:
            ...
            await page.screenshot(path="tradeinn_verification_two_phase_success.png")
            
        except Exception as e:
            print(f"❌ FINAL: No se detectaron productos: {str(e)[:100]}")
            await page.screenshot(path="tradeinn_verification_two_phase_fail.png")
            
        await browser.close()

if __name__ == "__main__":
    asyncio.run(verify_tradeinn_two_phase())
