    ...
    def add_offer(self, product: ProductModel, offer_data: dict, commit: bool = True) -> tuple[OfferModel, Optional[float]]:
        ...
        if existing_offer:
            ...
            if abs(existing_offer.price - current_price) > 0.01:
                ...
                if not last_snapshot or (datetime.now(timezone.utc).replace(tzinfo=None) - last_snapshot.recorded_at).total_seconds() > 86400:
            ...
            existing_offer.last_seen = datetime.now(timezone.utc).replace(tzinfo=None)
            ...
            new_offer = OfferModel(
                ...
                # Phase 41: Market Intelligence
                first_seen_at=offer_data.get("first_seen_at", datetime.now(timezone.utc).replace(tzinfo=None)),
                ...
                last_price_update=offer_data.get("last_price_update", datetime.now(timezone.utc).replace(tzinfo=None))
            )
