...
class MotuClassicsDEScraper(BaseScraper):
    """
    Scraper for MotU-Classics.de (Germany) - Specialist MOTU European Shop.
    Phase 8.4: Expansión Continental.
    """
    def __init__(self):
        super().__init__(
            shop_name="MotuClassicsDE",
            base_url="https://www.motu-origins.de/"
        )

    async def search(self, query: str = "auto") -> List[ScrapedOffer]:
        """
        Executes the scraping logic for MotU-Classics.de.
        """
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                while current_url and page_num <= max_pages:
                    ...
                    # German shops often use Shopware or WooCommerce
                    items = soup.select('.product-box, .product-card, .product--box, .product')
                    ...
                    # Pagination (Shopware pattern)
                    next_btn = soup.select_one('.pagination-next a, a.next, [rel="next"]')
                    if next_btn and next_btn.get('href'):
                        next_href = next_btn.get('href')
                        current_url = next_href if next_href.startswith('http') else f"https://www.motu-origins.de{next_href}"
                        page_num += 1
                    else:
                        break
    ...
    def _parse_item(self, item) -> Optional[ScrapedOffer]:
        try:
            ...
            if not link.startswith('http'):
                link = f"https://www.motu-origins.de{link}"
            
            # Find name
            title_tag = item.select_one('.product-name, .product--title, h2, h3, .title')
            name = title_tag.get_text(strip=True) if title_tag else "Unknown"
            
            # Find price (German format: 12,95 €)
            price_tag = item.select_one('.product-price, .price, .product--price')
            price_val = 0.0
            if price_tag:
            ...
            # Availability
            is_avl = True
            if item.select_one('.sold-out, .not-available, .is--not-available'):
                is_avl = False
